...
export default function AuditPage() {
  ...
  const formatErrorMessage = (error) => {
    ...
    if (error.message.includes('contract')) {
      return {
        ...
        message: 'There was an error interacting with the smart contract. Please try again later.',
        type: 'error'
      };
    }

    // Account selection errors
    if (error.message.includes('account') || error.message.includes('selected')) {
      return {
    ...
    if (error.message.includes('provider')) {
      return {
        ...
        message: 'Unable to connect to the blockchain provider. Please ensure you have MetaMask installed and try again.',
        type: 'error'
      };
    }
  ...
  const getEntityRatings = async () => {
    try {
      ...
      setEntityRatings(addresses.map((address, i) => ({
        address,
    ...
    switch (activeSection) {
            ...
            <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50 hover:shadow-[0_0_15px_rgba(59,130,246,0.2)] transition-all duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-blue-400">{fundRequests.filter(r => !r.isApproved && !r.isRejected).length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">Total Transactions</span>
                  <span className="text-blue-400">{spendingRecords.length}</span>
                </div>
              </div>
            </div>
            <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50 hover:shadow-[0_0_15px_rgba(59,130,246,0.2)] transition-all duration-300">
              ...
              {bonusTime && (
                <div className="text-center">
                  <p className="text-gray-400 mb-2">Next Bonus Distribution in:</p>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-700/50 rounded-lg p-3">
                      <p className="text-2xl font-bold text-yellow-400">{bonusTime.hours}</p>
                      <p className="text-sm text-gray-400">Hours</p>
                    </div>
                    <div className="bg-gray-700/50 rounded-lg p-3">
                      <p className="text-2xl font-bold text-yellow-400">{bonusTime.minutes}</p>
        ...
        return (
          <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-white">Fund Issuance Records</h3>
              <button
            ...
            <div className="space-y-4">
              {issuedFunds.map((fund, index) => (
                <div key={index} className="bg-gray-700/30 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-gray-300">ID: {fund.id.toString()}</p>
                      <p className="text-gray-400 text-sm">Entity: {fund.entity}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-400 font-semibold">{ethers.formatEther(fund.amount)} ETH</p>
                      <p className="text-gray-500 text-sm">
                        {new Date(fund.timestamp.toNumber() * 1000).toLocaleString()}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      case 'spending':
        return (
          <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-white">Spending Records</h3>
              <button
            ...
            <div className="space-y-4">
              {spendingRecords.map((record, index) => (
                <div key={index} className="bg-gray-700/30 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-gray-300">ID: {record.id.toString()}</p>
                      <p className="text-gray-400 text-sm">Entity: {record.entity}</p>
                      <p className="text-gray-400">{record.purpose}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-400 font-semibold">{ethers.formatEther(record.amount)} ETH</p>
                      <p className="text-gray-500 text-sm">Hash: {record.documentHash.slice(0, 10)}...</p>
                      <p className="text-gray-500 text-sm">
      ...
      case 'micro':
        return (
          <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-white">Micro-Transactions</h3>
              <button
                ...
                <div key={index} className="bg-gray-700/30 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">Record ID: {tx.spendingRecordId.toString()}</p>
                      <p className="text-gray-400">{tx.description}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-400 font-semibold">{ethers.formatEther(tx.amount)} ETH</p>
                      <p className="text-gray-500 text-sm">
      ...
      case 'requests':
        return (
          <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-white">Fund Requests</h3>
              <button
            ...
            <div className="space-y-4">
              {fundRequests.map((request, index) => (
                <div key={index} className="bg-gray-700/30 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-gray-300">ID: {request.id.toString()}</p>
                      <p className="text-gray-400 text-sm">Entity: {request.entity}</p>
                      <p className="text-gray-400">{request.reason}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-400 font-semibold">{ethers.formatEther(request.amount)} ETH</p>
                      <p className={`text-sm ${
                        request.isApproved ? 'text-green-400' : 
                        request.isRejected ? 'text-red-400' : 
                        'text-yellow-400'
                      }`}>
      ...
      case 'ratings':
        return (
          <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-6 border border-gray-700/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-white">Entity Ratings</h3>
              <button
              ...
              {entityRatings.map((rating, index) => (
                <div key={index} className="bg-gray-700/30 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-yellow-400 font-semibold">{rating.rating}/5</span>
                  </div>
                  <p className="text-gray-400 text-sm mt-2">Total Votes: {rating.votes}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800">
      ...
      <nav className="bg-gray-800/50 backdrop-blur-sm border-b border-gray-700">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            <h1 className="text-xl font-bold text-white">Transaction Audit Dashboard</h1>
      ...
      <div className="flex">
        ...
        <div className="w-64 min-h-screen bg-gray-800/50 backdrop-blur-sm border-r border-gray-700">
          ...
          <div className="p-4 border-b border-gray-700">
            <h5 className="font-semibold text-white mb-2">Select Auditor Account</h5>
            <select 
            ...
            {connectionStatus && (
              <p className={`mt-2 text-sm ${connectionStatus.type === 'error' ? 'text-red-400' : connectionStatus.type === 'success' ? 'text-green-400' : 'text-yellow-400'}`}>
          ...
          {!contract ? (
            <div className="max-w-2xl mx-auto text-center">
              <div className="bg-gray-800/30 backdrop-blur-md rounded-xl p-8 border border-gray-700/50">
                <h2 className="text-2xl font-bold text-white mb-4">Welcome to the Audit Dashboard</h2>
                <p className="text-gray-300 mb-6">
                  Please select an auditor account and connect to access the dashboard features.
                </p>
      ...
      {error && (
        <div className={`fixed bottom-4 right-4 max-w-md backdrop-blur-sm border px-4 py-3 rounded-lg shadow-lg transition-all duration-300 ${
          error.type === 'error' 
