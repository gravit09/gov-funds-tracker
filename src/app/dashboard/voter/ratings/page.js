      ...
      <nav className="bg-gray-800/50 backdrop-blur-sm border-b border-gray-700">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            <h1 className="text-xl font-bold text-white">Entity Performance Ratings</h1>
        ...
        <div className="flex-1 p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700">
              <div className="p-6">
                <h2 className="text-2xl font-bold text-white mb-6">All Entity Ratings</h2>
                <button 
                    ...
                    allRatings.map((rating, index) => (
                      <div key={index} className="bg-gray-700/50 p-4 rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                          <p className="text-gray-300 font-medium">Entity {index + 1}</p>
                          <div className="flex items-center">
                            <span className="text-yellow-400 mr-2">{rating.rating}/5</span>
                            <span className="text-gray-400">({rating.votes} votes)</span>
                          </div>
                        </div>
                        <p className="text-gray-400 text-sm break-all">{rating.address}</p>
                      </div>
                    ))
                  ) : (
                    <p className="text-gray-400 text-center py-4">No ratings available. Click "Get All Ratings" to view.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700">
              <div className="p-6">
                <h2 className="text-2xl font-bold text-white mb-6">Specific Entity Rating</h2>
                
                <div className="space-y-6">
                  <div>
                    <label className="block text-gray-300 mb-2">Entity Address</label>
                    <input
                      type="text"
                  ...
                  {entityRating && (
                    <div className="bg-gray-700/50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex items-center">
                          <span className="text-yellow-400 mr-2">{entityRating.rating}/5</span>
                          <span className="text-gray-400">({entityRating.totalVotes} votes)</span>
                        </div>
                      </div>
                      <p className="text-gray-400 text-sm">Last Vote Time: {entityRating.lastVoteTime}</p>
                    </div>
                  )}
                </div>
              </div>
